/**
 * Test using BIP-340 Schnorr from @noble/curves
 * This should be compatible with Nutshell's libsecp256k1
 */
...
console.log('\n🧪 Testing with BIP-340 Schnorr from @noble/curves\n');
...
async function main() {
  ...
  // Sign using BIP-340 Schnorr from @noble/curves
  console.log('Signing with BIP-340 Schnorr (@noble/curves/secp256k1)...');
  ...
  // Verify locally
  const verifyResult = secp.schnorr.verify(signatureBytes, messageHash, pubkey);
  ...
  try {
    ...
    console.log('🎉 BIP-340 Schnorr signatures work with Nutshell!\n');
